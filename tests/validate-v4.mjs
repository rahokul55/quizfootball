...
const html=fs.readFileSync(new URL('../index.html',import.meta.url),'utf8');ok(!/Rahim/i.test(html),'personal placeholder Rahim present');for(const x of [4,6,8])ok(html.includes(`<option value="${x}"`),`choice ${x} missing`);ok(html.includes('images-v4.js')&&html.includes('preflight-v4.js')&&html.includes('enhancer-v4.js'),'V4 layer not wired');
const pre=fs.readFileSync(new URL('../preflight-v4.js',import.meta.url),'utf8'),enh=fs.readFileSync(new URL('../enhancer-v4.js',import.meta.url),'utf8');ok(pre.includes('start()')&&pre.includes('gameScreen'),'full-match music observer missing');ok(enh.includes('choiceSegmentsV4'),'visible 4/6/8 controls missing');ok(enh.includes('v4VolumeInput'),'music volume control missing');ok(enh.includes('fitImage'),'image fit protection missing');
