...
async function jfetch(url,tries=4){for(let i=0;i<tries;i++){try{const r=await fetch(url,{headers:{'user-agent':'quizfootball-image-resolver/4.2'}});if(r.ok)return await r.json();if(r.status===429)await sleep(1200*(i+1));}catch{}await sleep(350*(i+1))}return null}
...
async function pool(entries,worker,n=7){let i=0;async function run(){while(i<entries.length){const e=entries[i++];await worker(e)}}await Promise.all(Array.from({length:n},run))}

await pool(Object.entries(D.teams),async([id,t])=>{if(out.teams[id])return;let u=await wiki(t.wiki,true,360);if(!u)u=await wiki(t.wiki,false,360);if(u)out.teams[id]=u;else missing.teams.push(id)});
await pool(Object.entries(D.players),async([id,p])=>{if(out.players[id])return;let u=await wiki(p.search,false,320);if(!u)u=await sportsPlayer(p.name);if(u)out.players[id]=u;else missing.players.push(id)},6);
